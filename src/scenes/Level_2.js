...
*Modification log: *Created Level 2 and pasted Level 1
                   *Changed maps for Level 2
                   *Added additional enemy to Level 2
...
export default class Level_2 extends Phaser.Scene {
  ...
  create() {
    /************
        MAP
    ************/
    map = this.make.tilemap({ key: "Level_2" });
    ...
    ladder = map.createLayer("Ladder", tileset);
    ...
    ************/
    ...
    this.physics.add.collider(player, ladder, goingUp);
  ...
  update() {
    ...
    ************/
    ...
    if (player.body.x > 430 && player.body.x < 438 && cursors.up.isDown) {
      player.setVelocityY(-200);
    }
    if (player.body.x > 200 && player.body.y < 30 && !enemy2.body.enable) {
      door = 1;
    }
